...
import cn.org.rapid_framework.generator.provider.db.table.model.CopyColumn;
				...
				for (Iterator<Column> iterator2 = columnsSet.iterator(); iterator2.hasNext();) {
					...
					if (column.getForeignInfo() != null && column.getForeignInfo().getRefer() != null && column.getForeignInfo().getRefer().trim().length() != 0) {
						...
						if (column.getForeignInfo().getCopyColumns() != null && column.getForeignInfo().getCopyColumns().size() > 0) {
							for (CopyColumn copy : column.getForeignInfo().getCopyColumns()) {
								String srcColumn = copy.getSrcColumn();
								String destColumn = copy.getDestColumn();
								copy.setDestinationColumn(table.getColumnByName(destColumn));
								if (referTable != null) {
									copy.setSourceColumn(referTable.getColumnByName(srcColumn));
								}
							}
	...
	private void setupReferColumn(ForeignInfo referredForeignInfo, Table table) {
		...
		if (referredForeignInfo.getForeignColumns() != null && referredForeignInfo.getForeignColumns().size() > 0) {
			for (Iterator<ForeignColumn> it = referredForeignInfo.getForeignColumns().iterator(); it.hasNext();) {
				...
				if (colRefer == null) {
					String errmsg = " Table:" + table.getSqlName() + ".foreign-info['" + foreignInfoId + "']." + foreignColumn.getRefer()
							+ " don't have any 'refer' attribute which reference to table '" + table.getSqlName() + "' ,please add 'refer' attibute .  ";
					GLogger.warn(errmsg);
					continue;
				}
				Column originalColumn = table.getColumnByName(colRefer);
				if (originalColumn == null) {
					String errmsg = " Table:" + table.getSqlName() + ".foreign-info['" + foreignInfoId + "']." + foreignColumn.getRefer() + " don't have any column name '" + foreignColumn.getRefer()
							+ "' define in table  " + table.getSqlName();
					GLogger.warn(errmsg);
					continue;
