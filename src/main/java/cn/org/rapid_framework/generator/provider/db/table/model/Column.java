...
public class Column implements java.io.Serializable, Cloneable {
	 ...
	 * @param remarks
	 */
	public Column(Table table, int sqlType, String sqlTypeName, String sqlName, int size, int decimalDigits, boolean isPk, boolean isNullable, boolean isIndexed, boolean isUnique,
	 ...
	 */
	public Column(Table table, int sqlType, String sqlTypeName, String sqlName, int size, int decimalDigits, boolean isPk, boolean isNullable, boolean isIndexed, boolean isUnique,
			String defaultValue, String remarks, Column columnFromXml) {
		if (sqlName == null)
			throw new NullPointerException();
		_table = table;
		_sqlType = sqlType;
		...
		if (columnFromXml != null) {
			this.override(columnFromXml);
		}
		initOtherProperties();
	}

	public Column(Column c) {
		this(c.getTable(), c.getSqlType(), c.getSqlTypeName(), c.getSqlName(), c.getSize(), c.getDecimalDigits(), c.isPk(), c.isNullable(), c.isIndexed(), c.isUnique(), c.getDefaultValue(), c
				.getRemarks());
	}
	...
	public String getValidateString() {
		return isNullable() ? getNoRequiredValidateString() : ColumnHelper.BLANK + "required" + getNoRequiredValidateString();
	}
	...
	public String getJavascriptValidatorExprssion() {
		return javascriptValidatorExprssion;
	}

	public void setJavascriptValidatorExprssion(String javascriptValidatorExprssion) {
	...
	public List<EnumMetaDada> getEnumListExcludeOtherVal() {
		List<EnumMetaDada> list = getEnumList();
		List<EnumMetaDada> list2 = new LinkedList();
	...
	@XStreamAlias("javascriptValidatorExprssion")
	@XStreamAsAttribute
	...
	public void override(Column column) {
		...
		this.javascriptValidatorExprssion = column.getJavascriptValidatorExprssion();
	...
	public String getEnumOtherVal() {
		if (enumOtherVal == null)
			return DEFAULT_ENUM_OTHER_VAL;
		else
			return enumOtherVal;
	}
